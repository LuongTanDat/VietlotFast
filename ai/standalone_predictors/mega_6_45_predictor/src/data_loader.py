...
def parse_optional_special(value: Any) -> int | None:
    text = str(value or "").strip()
    if not text:
        return None
    special = int(text)
    return special if 1 <= special <= 45 else None
...
def parse_main_numbers(raw_value: Any) -> tuple[int, ...]:
    ...
    if any(value < 1 or value > 45 for value in values):
        raise ValueError(f"Main numbers must be between 1 and 45: {raw_value!r}.")
    return tuple(values)


def parse_actual_numbers(raw_value: Any) -> tuple[int, ...]:
...
def _validate_game_label(label: str) -> None:
    ...
    if normalized and "mega" not in normalized and "6/45" not in normalized:
        raise ValueError(f"Unexpected game label for Mega 6/45 dataset: {label!r}")
...
def load_draw_records(csv_path: str | Path, column_mapping_path: str | Path | None = None) -> dict[str, Any]:
    ...
    time_slot_usable = bool(records) and (non_blank_time_count / len(records)) >= 0.4
    return {
        "records": records,
        "csv_path": csv_path,
...
def build_dataset_summary(bundle: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "label": latest.game_label if latest else "Mega_6/45",
    }
...
def infer_next_draw(records: list[DrawRecord], schedule_weekdays: list[int] | tuple[int, ...]) -> dict[str, Any]:
    if not records:
...
def draw_to_dict(record: DrawRecord) -> dict[str, Any]:
    return {
        ...
        "display_lines": record.display_lines,
