...
def build_pair_context(draws: list[Any], history_limit: int = 180) -> dict[str, Any]:
    ...
    top_pairs = sorted(
        pair_stats.items(),
        key=lambda item: (item[1]["lift"], item[1]["pair_count"]),
        reverse=True,
    )[:12]
    return {
...
def score_number(number: int, anchor_numbers: list[int], pair_context: dict[str, Any]) -> float:
    ...
    for anchor in anchors:
        ...
        values.append(max(0.0, min(1.0, 0.35 + 0.25 * confidence + 0.20 * min(2.5, lift) / 2.5)))
...
def anti_pair_adjustment(number: int, anchor_numbers: list[int], pair_context: dict[str, Any]) -> float:
    ...
    for anchor in anchors:
        ...
        lift = float(stats.get("lift", 1.0))
        if lift >= 1.0:
            values.append(min(1.0, 0.55 + min(1.5, lift - 1.0) * 0.25))
        else:
            values.append(max(0.0, 0.55 - (1.0 - lift) * 0.65))
    return sum(values) / len(values)
...
def score_ticket(ticket_numbers: list[int] | tuple[int, ...], pair_context: dict[str, Any]) -> dict[str, float]:
    ...
    for number_a, number_b in combinations(ticket_numbers, 2):
        ...
        if not stats:
            compatibility_values.append(0.45)
            anti_pair_values.append(0.25)
            continue
        lift = float(stats.get("lift", 1.0))
        confidence = float(stats.get("confidence", 0.0))
        compatibility_values.append(max(0.0, min(1.0, 0.35 + 0.20 * confidence + 0.20 * min(2.5, lift) / 2.5)))
