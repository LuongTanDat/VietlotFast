...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Standalone Power 6/55 adaptive predictor.")
    ...
    update_parser.add_argument("--csv", default="data/power_6_55.csv", help="Path to the Power 6/55 CSV history.")
    update_parser.add_argument("--actual-main", default=None, help='Optional actual main numbers such as "09,21,32,34,52,53".')
    update_parser.add_argument("--actual-special", default=None, help='Optional actual special number such as "22".')

    backtest_parser = subparsers.add_parser("backtest", help="Run chronological backtesting.")
    backtest_parser.add_argument("--csv", default="data/power_6_55.csv", help="Path to the Power 6/55 CSV history.")
    ...
    backtest_parser.add_argument("--min-history", default=80, type=int, help="Minimum historical draws before first fold.")
    ...
    retrain_parser.add_argument("--csv", default="data/power_6_55.csv", help="Path to the Power 6/55 CSV history.")

    deep_status_parser = subparsers.add_parser("deep-status", help="Inspect whether the real deep model is active.")
    deep_status_parser.add_argument("--csv", default="data/power_6_55.csv", help="Optional CSV path used for context.")

    blend_status_parser = subparsers.add_parser("blend-status", help="Inspect the active blend profiles and latest ablation report.")
    blend_status_parser.add_argument("--csv", default="data/power_6_55.csv", help="Optional CSV path used for context.")

    audit_parser = subparsers.add_parser("audit-assembly", help="Inspect disagreement, assembly modes, and the selected ticket.")
    audit_parser.add_argument("--csv", default="data/power_6_55.csv", help="Path to the Power 6/55 CSV history.")
    return parser
...
def main() -> int:
    ...
    if args.command == "predict":
        ...
        payload = predictor_api.update_after_actual(
            args.csv,
            actual_main_raw=args.actual_main,
            actual_special_raw=args.actual_special,
