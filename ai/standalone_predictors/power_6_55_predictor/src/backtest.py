...
def _evaluate_mode(
    ...
    for fold_number, index in enumerate(indexes, start=1):
        ...
        final_scores = dict((prediction.get("scoring_context") or {}).get("final_main_scores") or {})
        calibrated = scores_to_probabilities(final_scores, 6, 1, 55) if final_scores else {}
        if calibrated:
        ...
        folds.append({
            ...
            "special_hit": int(prediction["special"] == actual.special),
    ...
    brier = brier_score(probability_rows, label_rows, 55) if probability_rows else 0.0
    ll = log_loss(probability_rows, label_rows, 55) if probability_rows else 0.0
    cal = calibration_error(probability_rows, label_rows, 55) if probability_rows else 0.0
