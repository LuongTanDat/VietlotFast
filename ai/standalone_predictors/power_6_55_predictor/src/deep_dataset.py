...
FEATURE_SCHEMA_VERSION = "power_6_55_deep_features_v1"
...
def _recent_frequency(draws: list[Any], number_max: int) -> dict[int, float]:
    ...
    return feature_engineering.build_number_frequency(draws, number_max=number_max)


def _recent_special_frequency(draws: list[Any], number_max: int) -> dict[int, float]:
    if not draws:
        return {number: 0.0 for number in range(1, number_max + 1)}
    return feature_engineering.build_special_frequency(draws, number_max=number_max)
...
def _step_features(draws: list[Any], index: int, time_slot_enabled: bool) -> tuple[list[float], list[str]]:
    ...
    recent_secondary = draws[max(0, index - 12) : index]
    recent_union = {number for scoped in recent_primary for number in scoped.main_numbers}
    recent_special_values = [scoped.special for scoped in recent_primary]
    recent_frequency = _recent_frequency(recent_secondary, 55)
    recent_special_frequency = _recent_special_frequency(recent_secondary, 55)

    values: list[float] = []
    names: list[str] = []

    for position, value in enumerate(_multi_hot(draw.main_numbers, 55), start=1):
        names.append(f"main_hot_{position}")
        values.append(float(value))
    for position, value in enumerate(_one_hot(int(draw.special) - 1, 55), start=1):
        names.append(f"special_hot_{position}")
        values.append(float(value))

    for offset, number in enumerate(draw.main_numbers, start=1):
        names.append(f"position_{offset}")
        values.append(float(number) / 55.0)
    for offset in range(5):
        names.append(f"gap_{offset + 1}_{offset + 2}")
        values.append(float(draw.main_numbers[offset + 1] - draw.main_numbers[offset]) / 55.0)

    summary_values = {
        "sum_main": sum(draw.main_numbers) / (55.0 * 6.0),
        "mean_main": (sum(draw.main_numbers) / 6.0) / 55.0,
        "min_main": min(draw.main_numbers) / 55.0,
        "max_main": max(draw.main_numbers) / 55.0,
        "range_main": (max(draw.main_numbers) - min(draw.main_numbers)) / 55.0,
        "special_number": float(draw.special) / 55.0,
    ...
    for modulus in (2, 3, 5, 10, 11):
        for residue, value in enumerate(_count_residues(draw.main_numbers, modulus)):
            names.append(f"mod{modulus}_{residue}")
            values.append(float(value))

    special_residue_payload = {
        "special_mod2": float(draw.special % 2),
        "special_mod3": float(draw.special % 3) / 2.0,
        "special_mod5": float(draw.special % 5) / 4.0,
        "special_mod10": float(draw.special % 10) / 9.0,
        "special_repeat_previous": float(previous_draw is not None and draw.special == previous_draw.special),
        "special_in_recent_window": float(draw.special in set(recent_special_values)),
    }
    for key, value in special_residue_payload.items():
        names.append(key)
        values.append(float(value))
    ...
    recent_special_activity = recent_special_frequency.get(int(draw.special), 0.0)
    summary_overlap = {
        ...
        "recent_special_activity": recent_special_activity,
        "recent_window_fill": min(1.0, len(recent_secondary) / 12.0),
    }
...
def _regime_target(history: list[Any]) -> int:
    recent_frequency = feature_engineering.build_number_frequency(history[-10:], number_max=55) if history else {}
...
def build_training_samples(
    ...
    config_sequence_length = int(((predictor_config.get("deep_training") or {}).get("sequence_length", 12)) or 12)
    sequence_length = max(4, int(sequence_length or config_sequence_length))
    if len(draws) <= sequence_length:
        raise ValueError("Not enough Power 6/55 draws to build deep-learning sequence samples.")
    ...
    for target_index in range(sequence_length, len(draws)):
        ...
        targets_main.append(np.asarray(_multi_hot(target_draw.main_numbers, 55), dtype=np.float32))
        targets_special.append(int(target_draw.special) - 1)
    ...
    return {
        ...
        "targets_extra": np.asarray(targets_special, dtype=np.int64),
...
def build_inference_sample(
    ...
    sequence_length = max(4, int(sequence_length or config_sequence_length))
    if len(draws) < sequence_length:
        raise ValueError("Not enough Power 6/55 draws to build the deep inference sequence.")
