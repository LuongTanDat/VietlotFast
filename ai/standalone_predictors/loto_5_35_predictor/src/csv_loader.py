...
def normalize_game(game: str = SUPPORTED_GAME) -> str:
    ...
    if normalized not in {SUPPORTED_GAME, "loto5/35", "loto_5_35"}:
        raise ValueError("loto_5_35_predictor hiện chỉ hỗ trợ loto_5_35.")
...
def resolve_csv_path(csv_path: str | Path | None = None) -> Path:
    if csv_path in {None, ""}:
        return cfg.DATA_DIR / "loto_5_35.csv"
    path = Path(csv_path)
    if path.is_absolute():
        return path
    if path.exists():
        return path.resolve()
    candidate = (cfg.PROJECT_ROOT / path).resolve()
    return candidate
...
def load_history(game: str = SUPPORTED_GAME, csv_path: str | Path | None = None):
    normalize_game(game)
...
def build_sync_summary(draws):
    return build_sync_summary_for_path(draws, resolve_csv_path())


def build_sync_summary_for_path(draws, csv_path: str | Path | None = None):
    path = resolve_csv_path(csv_path)
