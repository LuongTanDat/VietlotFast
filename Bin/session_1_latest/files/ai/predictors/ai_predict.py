...
GEN_LOCAL_MODEL_DIR = dp.MODELS_DIR
...
def build_loto_6_45_vip_prediction(bundle_count, requested_engine="", risk_mode="balanced"):
    standalone_root = PROJECT_ROOT / "mega_6_45_predictor"
...
def _run_standalone_predict_cli(standalone_root, csv_path):
...
def build_loto_6_55_vip_prediction(bundle_count, requested_engine="", risk_mode="balanced"):
    standalone_root = PROJECT_ROOT / "power_6_55_predictor"
...
def get_gen_local_model_path(type_key):
    return dp.get_gen_model_path(type_key)
...
def get_online_meta_model_path(type_key):
    return dp.get_meta_model_path(type_key)
...
def predict_json(type_key, bundle_count, keno_level=None, engine=AI_ENGINE_CLASSIC, risk_mode=AI_RISK_MODE_BALANCED, prediction_mode=PREDICTION_MODE_NORMAL):
    ...
    if type_key == "LOTO_5_35" and prediction_mode == PREDICTION_MODE_VIP:
        try:
            ...
            fallback_prefix = f"predictor_v2 lỗi, đang dùng Vip fallback: {exc}"
            if engine == AI_ENGINE_LUAN_SO:
...
def export_special_scoring_csv(type_key, draws, recent_window, weights, co_top_k):
    ...
    return {
        ...
        "csvRelativePath": str(csv_path.relative_to(dp.PROJECT_ROOT)).replace("\\", "/"),
...
def score_json(type_key, recent_window=None, weights=None, co_top_k=None, include_backtest=False, backtest_top_k=None,
    ...
    if export_csv:
        ...
        payload["csvRelativePath"] = str(csv_path.relative_to(dp.PROJECT_ROOT)).replace("\\", "/")
