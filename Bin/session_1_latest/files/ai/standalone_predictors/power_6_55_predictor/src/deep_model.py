...
MODEL_VERSION = "power_6_55_deep_v1"


def _empty_main_scores() -> dict[int, float]:
    return {number: 0.0 for number in range(1, 56)}


def _empty_special_scores() -> dict[int, float]:
    return {number: 0.0 for number in range(1, 56)}
...
def _deep_config(predictor_config: dict[str, Any]) -> dict[str, Any]:
    ...
    payload["head_weights"] = {"main": 1.0, "extra": 0.38, "regime": 0.18}
    payload["ticket_size"] = int((((predictor_config or {}).get("numbers") or {}).get("main_count", 6)) or 6)
    payload["top_k"] = max(payload["ticket_size"], int((predictor_config or {}).get("top_candidate_count", 16) or 16))
    return payload
...
def _inspect_artifacts(
    ...
    if not bool(feature_flags.get("use_deep_model", True)):
        ...
        status["deep_status_line"] = _status_line(False, "fallback_heuristic_only", status["deep_status_reason"], meta)
        return status
...
def train_from_csv(
    ...
    meta = {
        ...
        "metrics": {
            ...
            "special_loss": float(trained["metrics"]["train"]["extra_loss"] or 0.0),
            "val_special_loss": float(trained["metrics"]["validation"]["extra_loss"] or 0.0),
            ...
            "special_accuracy": float(trained["metrics"]["validation"]["extra_accuracy"] or 0.0),
...
def score_numbers(
    ...
    try:
        ...
        return {
            ...
            "main_scores": _empty_main_scores(),
            "special_scores": _empty_special_scores(),
    ...
    special_probabilities = outputs.get("extra_probs")
    ...
    special_scores = _empty_special_scores()
    if special_probabilities is not None:
        special_scores = {number: float(special_probabilities[0][number - 1]) for number in range(1, 56)}
    return {
        "available": True,
        "message": status["deep_status_line"],
        "main_scores": {number: float(main_probabilities[number - 1]) for number in range(1, 56)},
...
def fine_tune_after_actual(
    ...
    del actual_main_numbers, actual_special
